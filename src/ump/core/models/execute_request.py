...
class OutputSpec(BaseModel):
    format: Optional[str] = Field(None, description="Requested output format / media type")
    transmissionMode: Optional[TransmissionMode] = Field(None, description="inline (value) or reference")
            ...
            if isinstance(val, list):
                ...
                for item in val:
                    if not isinstance(item, InlineOrRef):
                        raise ValueError(f"Input '{key}' list contains non InlineOrRef item")
            elif not isinstance(val, InlineOrRef):
                raise ValueError(f"Input '{key}' must be InlineOrRef or list[InlineOrRef]")
        return v
    ...
    def as_provider_payload(self) -> Dict[str, Any]:
        ...
        for key, val in self.inputs.items():
            if isinstance(val, list):
                ...
                inputs_payload[key] = val.value if val.is_inline else {"href": str(val.href)}
